...
function mapShoppingItem(x){
  ...
  return {
    ...
    thumbnail: x.thumbnail || undefined,            // <— grab SerpAPI thumbnail when present
  };
}
...
// tiny in-memory cache to avoid re-scraping the same pages during a session
...
function normalizeHttps(u) {
  try { return u.replace(/^http:\/\//i, "https://"); } catch { return u; }
}
    ...
    for (const cat of categories) {
      ...
      for (const it of items) {
        ...
        kept.push({
          ...
          source: it.source,
          thumbnail: it.thumbnail,  // pass through for resolver
