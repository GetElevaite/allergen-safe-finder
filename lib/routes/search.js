...
/* Strict screening: require sufficient ingredient text to trust the screen */
function hasSufficientIngredientData(inciText) {
  if (!inciText) return false;
  if (inciText.length < 20) return false;                 // too short to be a real list
  if (!/[;,]/.test(inciText)) return false;               // most INCI lists have separators
  return true;
}
  ...
  try {
    ...
    for (const cat of categories) {
      ...
      for (const it of items) {
        ...
        // ingredients (STRICT: require sufficient INCI; exclude if absent/ambiguous)
        const inci = await fetchIngredients(it.link);
        if (!hasSufficientIngredientData(inci)) continue;          // <-- conservative drop
    ...
    res.json({
      ...
      results,
      message:
        `Strict screen enabled — only items with sufficient ingredient data and no matches to (${allergens.join(", ") || "none"})` +
        ((priceMin!==null||priceMax!==null) ? ` • Price${priceMin!==null?` ≥ $${priceMin}`:""}${priceMax!==null?` ≤ $${priceMax}`:""}` : "") +
