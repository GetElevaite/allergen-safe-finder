...
function expandAllergens(input=[]) {
  ...
  for (const raw of input) {
    ...
    set.add(k); (ALLERGEN_SYNONYMS[k] || []).forEach(s => set.add(norm(s)));
  }
  return [...set];
}
...
async function resolveLocationCanonical(userLoc) {
  ...
  try {
    ...
    // prefer canonical_name if present; fall back to name
...
async function serpShopping(q, locationCanonical){
  ...
  if (locationCanonical) url.searchParams.set("location", locationCanonical);
  url.searchParams.set("api_key", SERP);
...
function cacheSet(k, v) { ogImageCache.set(k, v); if (ogImageCache.size > MAX_CACHE) ogImageCache.delete(ogImageCache.keys().next().value); }
...
async function resolveImageForItem(item) {
  ...
  if (!link) return null;
  const cached = cacheGet(link); if (cached !== undefined) return cached;
  try {
    ...
    cacheSet(link, url);
    return url;
  } catch { cacheSet(link, null); return null; }
}
...
router.post("/", async (req, res) => {
  try {
    ...
    // NEW: canonicalize location for Google Shopping
